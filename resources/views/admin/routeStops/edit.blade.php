...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.route-stops.update", [$routeStop->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.routeStop.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $routeStop->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fare') ? 'is-invalid' : '' }}" type="number" name="fare" id="fare" value="{{ old('fare', $routeStop->fare) }}" step="0.01">
                @if($errors->has('fare'))
