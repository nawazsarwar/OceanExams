    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subjects.update", [$subject->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" type="text" name="remarks" id="remarks" value="{{ old('remarks', $subject->remarks) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sections') ? 'is-invalid' : '' }}" name="sections[]" id="sections" multiple required>
                    @foreach($sections as $id => $section)
                        <option value="{{ $id }}" {{ (in_array($id, old('sections', [])) || $subject->sections->contains($id)) ? 'selected' : '' }}>{{ $section }}</option>
                    @endforeach
                </select>
