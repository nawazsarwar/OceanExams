...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.fee-heads.update", [$feeHead->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.feeHead.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $feeHead->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('recurrence') ? 'is-invalid' : '' }}" name="recurrence" id="recurrence">{{ old('recurrence', $feeHead->recurrence) }}</textarea>
