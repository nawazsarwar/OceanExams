...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fileModeOnlineTest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.file-mode-online-tests.update", [$fileModeOnlineTest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="quiz">{{ trans('cruds.fileModeOnlineTest.fields.quiz') }}</label>
                <input class="form-control {{ $errors->has('quiz') ? 'is-invalid' : '' }}" type="text" name="quiz" id="quiz" value="{{ old('quiz', $fileModeOnlineTest->quiz) }}" required>
                @if($errors->has('quiz'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\FileModeOnlineTest::MODE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('mode') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="mode_{{ $key }}" name="mode" value="{{ $key }}" {{ old('mode', $fileModeOnlineTest->mode) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $fileModeOnlineTest->type) }}">
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('test_date') ? 'is-invalid' : '' }}" type="text" name="test_date" id="test_date" value="{{ old('test_date', $fileModeOnlineTest->test_date) }}">
