        ...
        <div class="col-md-12">
            @can('transport_route_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.transport-routes.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.transportRoute.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'TransportRoute', 'route' => 'admin.transport-routes.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                                    ...
                                    <tr data-entry-id="{{ $transportRoute->id }}">
                                        ...
                                        <td>
                                            @can('transport_route_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.transport-routes.show', $transportRoute->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('transport_route_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.transport-routes.edit', $transportRoute->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('transport_route_delete')
                                                <form action="{{ route('frontend.transport-routes.destroy', $transportRoute->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('transport_route_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.transport-routes.massDestroy') }}",
