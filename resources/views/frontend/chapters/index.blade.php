        ...
        <div class="col-md-12">
            @can('chapter_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.chapters.create') }}">
                            ...
                            <tbody>
                                @foreach($chapters as $key => $chapter)
                                    <tr data-entry-id="{{ $chapter->id }}">
                                        ...
                                        <td>
                                            @can('chapter_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.chapters.show', $chapter->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('chapter_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.chapters.edit', $chapter->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('chapter_delete')
                                                <form action="{{ route('frontend.chapters.destroy', $chapter->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('chapter_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.chapters.massDestroy') }}",
